        ...
        for (WebSocketSession session : sessions.values()) {
            if (session.isOpen()) {
                try {
                    ...
                    log.error("YJS 메세지 전송 실패 projectFileId = {}, session = {}", projectFileId, session,e);
                }
            } else {
                log.warn("세션이 열려 있지 않음 projectFileId = {}, session = {}", projectFileId, session); // 세션 열려 있지 않을 경우 경고 로깅
            }
        }
    }
